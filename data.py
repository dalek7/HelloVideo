...
class DataLoader():
    ...
    def __init__(self, data_dir, n_videos={'train': None, 'validation': None},
        ...
        self.video_dir = os.path.join(self.data_dir, 'data')
...
class CNN3DDataLoader(DataLoader):
    ...
    def adjust_sequence_length(self, frame_files):
        ...
        if frame_diff == 0:
            ...
            #return frame_files[frame_diff//2:(frame_diff//2)+self.seq_length]
            
            # Cuts off last few frames
            #return frame_files[:self.seq_length]
            
        else:
            # Repeats the first frame to lengthen video --> no prob.
            return frame_files[:1] * abs(frame_diff) + frame_files
            
            # Repeats the last frame to lengthen video --> no prob.
            #return frame_files[-1:] * abs(frame_diff) + frame_files        
